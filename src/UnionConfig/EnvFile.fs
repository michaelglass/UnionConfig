...
/// Format the kind tag for a config var definition
let private formatKindTag =
    function
    | Manual -> "Manual"
...
let missingEntriesHeader (defs: ConfigVarDef array) (values: Map<string, string>) : string list =
    let missing =
        defs
        |> Array.filter (fun d ->
