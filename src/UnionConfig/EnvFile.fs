...
/// Write a sectioned .env file with optional header lines and sections
/// Header lines are written as-is before sections. Each section gets a `# === Header ===` header line.
let writeEnvFile (path: string) (headerLines: string list) (sections: EnvFileSection array) : unit =
    ...
    for section in sections do
        if not firstSection || not (List.isEmpty headerLines) then
...
let defaultSections
    (groupedDefs: (string * ConfigVarDef<'FetchSource> array) array)
...
/// Format the provenance tag for a config var definition. Depends only on the case
/// shape, so it needs no constraint on `'FetchSource` (the source detail is omitted).
let private formatProvenanceTag (provenance: Provenance<'FetchSource>) : string =
    match provenance with
    | Operator -> "Operator"
    | SystemGenerated -> "SystemGenerated"
    | Provisioned Cached -> "Provisioned(Cached)"
    | Provisioned(Fetched _) -> "Provisioned(Fetched)"
    | Ambient -> "Ambient"

/// Generate comment lines summarizing missing required config entries.
/// Returns empty list when nothing is missing.
let missingEntriesHeader (defs: ConfigVarDef<'FetchSource> array) (values: Map<string, string>) : string list =
    let missing =
        defs
        |> Array.filter (fun d ->
            d.Requirement = Required
            && (values |> Map.tryFind d.Name |> Option.defaultValue "" |> String.IsNullOrEmpty))
        |> Array.toList
