...
SSM or `.env`. Reading works like any other kind:
...
UnionConfig does not enforce non-persistence, so filter these out before writing
to a store using `ConfigVarKind.isPersistable`:

<!-- sync:external:start src=examples/ExampleApp/Program.fs -->
```fsharp
