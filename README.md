...
`DefaultValue` is the one knob for "static fallback value." It's used in two places:
...
- **`.env` / store seeders** (e.g. `populateDefaults`) write it as the initial
...
`Kind` and `DefaultValue` are orthogonal: `Kind` says *who owns the var* (user,
infra, runtime, …) and `DefaultValue` says *what to fall back to if no one set
it*. Use `DefaultValue = None` for vars with no static fallback.
<!-- sync:defaults:end -->
...
SSM or `.env`. Reading works like any other kind; persistence flows must filter them
out explicitly via `ConfigVarKind.isPersistable`:
