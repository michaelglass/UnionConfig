...
module ProvenanceTests =
    /// Concrete fetch source for exercising the generic provenance helpers.
    type private Src = StackOutput of name: string

    [<Fact>]
    let ``behavior Operator: persisted, editable, no fetch source`` () =
        let b = Provenance.behavior (Operator: Provenance<Src>)
        test <@ b.IsPersisted = true @>
        test <@ b.AllowedMutation = Edit @>
        test <@ b.FetchSource = None @>

    [<Fact>]
    let ``behavior SystemGenerated: persisted, rotate, no fetch source`` () =
        let b = Provenance.behavior (SystemGenerated: Provenance<Src>)
        test <@ b.IsPersisted = true @>
        test <@ b.AllowedMutation = Rotate @>
        test <@ b.FetchSource = None @>

    [<Fact>]
    let ``behavior Provisioned Cached: persisted, reprovision, no fetch source`` () =
        let b = Provenance.behavior (Provisioned Cached: Provenance<Src>)
        test <@ b.IsPersisted = true @>
        test <@ b.AllowedMutation = Reprovision @>
        test <@ b.FetchSource = None @>

    [<Fact>]
    let ``behavior Provisioned (Fetched src): NOT persisted, reprovision, Some src`` () =
        let src = StackOutput "ContentBucket"
        let b = Provenance.behavior (Provisioned(Fetched src))
        test <@ b.IsPersisted = false @>
        test <@ b.AllowedMutation = Reprovision @>
        test <@ b.FetchSource = Some src @>

    [<Fact>]
    let ``behavior Ambient: NOT persisted, no mutation, no fetch source`` () =
        let b = Provenance.behavior (Ambient: Provenance<Src>)
        test <@ b.IsPersisted = false @>
        test <@ b.AllowedMutation = NoMutation @>
        test <@ b.FetchSource = None @>

    [<Fact>]
    let ``isPersisted: true for cached provenances, false for fetched and ambient`` () =
        test <@ Provenance.isPersisted (Operator: Provenance<Src>) = true @>
        test <@ Provenance.isPersisted (SystemGenerated: Provenance<Src>) = true @>
        test <@ Provenance.isPersisted (Provisioned Cached: Provenance<Src>) = true @>
        test <@ Provenance.isPersisted (Provisioned(Fetched(StackOutput "x"))) = false @>
        test <@ Provenance.isPersisted (Ambient: Provenance<Src>) = false @>

    [<Fact>]
    let ``fetchSource: Some only for Fetched`` () =
        let src = StackOutput "Region"
        test <@ Provenance.fetchSource (Provisioned(Fetched src)) = Some src @>
        test <@ Provenance.fetchSource (Operator: Provenance<Src>) = None @>
        test <@ Provenance.fetchSource (Provisioned Cached: Provenance<Src>) = None @>
        test <@ Provenance.fetchSource (Ambient: Provenance<Src>) = None @>

    [<Fact>]
    let ``allowedMutation: derived per provenance`` () =
        test <@ Provenance.allowedMutation (Operator: Provenance<Src>) = Edit @>
        test <@ Provenance.allowedMutation (SystemGenerated: Provenance<Src>) = Rotate @>
        test <@ Provenance.allowedMutation (Provisioned Cached: Provenance<Src>) = Reprovision @>
        test <@ Provenance.allowedMutation (Provisioned(Fetched(StackOutput "x"))) = Reprovision @>
        test <@ Provenance.allowedMutation (Ambient: Provenance<Src>) = NoMutation @>

module DefaultTests =
    [<Fact>]
    let ``runtimeFallback: Some for RuntimeFallback and SeedAndFallback only`` () =
        test <@ Default.runtimeFallback (RuntimeFallback "a") = Some "a" @>
        test <@ Default.runtimeFallback (SeedAndFallback "b") = Some "b" @>
        test <@ Default.runtimeFallback NoDefault = None @>
        test <@ Default.runtimeFallback (SeedOnly "c") = None @>

    [<Fact>]
    let ``seed: Some for SeedOnly and SeedAndFallback only`` () =
        test <@ Default.seed (SeedOnly "a") = Some "a" @>
        test <@ Default.seed (SeedAndFallback "b") = Some "b" @>
        test <@ Default.seed NoDefault = None @>
        test <@ Default.seed (RuntimeFallback "c") = None @>
